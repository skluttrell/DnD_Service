...
def gen_url(type: str='integers', num: int=1, min: int=1, max: int=2, col: int=1, len: int=10, digits: str='on', upperalpha: str='on', loweralpha: str='on', unique: str='on', base: int=10, format: str='plain', rnd: str='new') -> str:
	"""Returns a random.org url for the requested data
...
def get_true_random(type: str='integers', num: int=1, min: int=1, max: int=2, col: int=1, len: int=10, digits: str='on', upperalpha: str='on', loweralpha: str='on', unique: str='on', base: int=10, format: str='plain', rnd: str='new') -> int or str:
	if get_quota() < 0: return get_pseudo_random(type, num, max, col, len, digits, upperalpha, loweralpha, unique, base)
	http = urllib3.PoolManager()
	requestURL = gen_url(type, num, min, max, col, len, digits, upperalpha, loweralpha, unique, base, format, rnd)
	request = http.request('GET', requestURL)
	if request.status == 200: return request.data.decode('utf-8').split()
	return -1

def get_pseudo_random(type: str='integers', num: int=1, min: int=1, max: int=2, col: int=1, len: int=10, digits: str='on', upperalpha: str='on', loweralpha: str='on', unique: str='on', base: int=10) -> int or str:
	if type == 'integers':
		integers = []
